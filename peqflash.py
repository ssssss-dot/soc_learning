...
#### STEP6: Boot CPU ####
try:
    # DMEM下载成功后，Loader已经返回IDLE，此时发送B0启动CPU
    time.sleep(0.01)

    ser.reset_input_buffer()
    ser.write(CMD_BOOT_REQ)
    ...
    if len(response) == 1:
        if response == SUCCESS:
            print("CPU booted successfully.")
        else:
            print(
                f"CPU boot failed with ERROR code = "
                f"0x{response.hex().upper()}"
            )
            sys.exit(1)
    else:
        print("CPU boot response timed out.")
        sys.exit(1)

except serial.SerialException:
    print(
        f"\npeqFlash>> [ERROR] "
        f"Serial port connection lost @{ser.name}!!"
    )
    exitfail()
except Exception as e:
    print(f"CPU boot failed: {e}")
    exitfail()
