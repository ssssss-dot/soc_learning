...
// 5个Crossbar输入接口：0=ICache读、1=DCache读写、2=Loader写，3=cpu主机， 4=dma
// 输入ID为6位，Crossbar追加2位源编号后形成8位输出ID。
taxi_axi_if #(
    .DATA_W (32),
    .ADDR_W (32),
    .ID_W   (`AXI_S_ID_WIDTH)
...
//Crossbar唯一输出接口预留给后续Clock Converter/MIG；MIG侧地址30位、ID 8位。
taxi_axi_if #(
    .DATA_W (32),
    .ADDR_W (30),
    .ID_W   (`AXI_M_ID_WIDTH)
...
// Bridge总封装：把ICache、DCache和Loader请求转换成Crossbar需要的AXI通道。
// 三个Bridge与CPU/Crossbar前端统一运行在IBUFDS输出的100MHz clk时钟域。
...
// 4输入2输出AXI Crossbar：仲裁ICache、DCache和Loader访问请求。
// 前端使用100MHz clk；30位解码窗口覆盖0x0000_0000至0x3FFF_FFFF。
